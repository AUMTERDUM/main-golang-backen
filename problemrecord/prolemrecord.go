...
func CreateProblemRecord(c *fiber.Ctx) error {
	...
	timebyuser,err := strconv.Atoi(c.FormValue("timebyuser"))
	if err != nil {
		fmt.Println(err)
	}
...
func CompletedProblemRecord(c *fiber.Ctx) error {
	...
	problemrecord := entities.ProblemRecord{
		...
		Status:        3,
		Time:          (currentTime.Sub(data_problem.CreatedAt)).Seconds(),
	}
	//fmt.Println((data_problem.CompletedAt).Sub(data_problem.CreatedAt).String())
	fmt.Println(data_problem.CompletedAt)
	...
	return c.JSON(fiber.Map{"casuseproblem": problemrecord.Casuseproblem, "solution": problemrecord.Solution, "suggestion": problemrecord.Suggestion, "status": problemrecord.Status, "completed_at": problemrecord.CompletedAt, "problem": problemrecord.Problem, "probelmtype": problemrecord.Problemtype, "message": "Update Successfully", "time": problemrecord.Time})
